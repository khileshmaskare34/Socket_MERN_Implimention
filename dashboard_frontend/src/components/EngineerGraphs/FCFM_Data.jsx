...
import FCFM_Graph from './FCFM_Graph'
var api_url = import.meta.env.VITE_API_URL;

const FCFM_Data = ({refreshGraphs}) => {
  ...
  const fetchData = async () => {
    ...
    if (filter === 'month') {
      ...
      url = `${api_url}/finalCheckedFolders-Graph?start_date=${start}&end_date=${end}`;
    } else if (filter === 'specific_date' && startDate && endDate) {
      url = `${api_url}/finalCheckedFolders-Graph?start_date=${startDate}&end_date=${endDate}`;
    } else if (filter === 'year') {
  ...
  return (
   <>
    ...
    <div className='card_top'>
      <h4>Total Final Checked Folders This</h4>
    ...
    {data.length > 0 ? (
            ...
            <p>No folders final checked this {filter === 'month' ? 'month' : 'date range'}.</p>
        )}
    </div>
  </div>
  ...
  <div className='graph-chart'>
    {data.length > 0 ? (
      <FCFM_Graph key={data.length} data={data} />  
    ) : (
      <FCFM_Graph key={0} data={[]} /> 
    )}
  </div>
 </>
  )
