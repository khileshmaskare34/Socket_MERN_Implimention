...
import CFM_Graph from './CFM_Graph'
var api_url = import.meta.env.VITE_API_URL;

const CFM_Data = ({refreshGraphs}) => {
  ...
  const fetchData = async () => {
    ...
    if (filter === 'month') {
      ...
      url = `${api_url}/checkedFolders-Graph?start_date=${start}&end_date=${end}`;
    } else if (filter === 'specific_date' && startDate && endDate) {
      url = `${api_url}/checkedFolders-Graph?start_date=${startDate}&end_date=${endDate}`;
    } else if (filter === 'year') {
    ...
    <>
        <div className="graph-data">
          <div className='card_top'>
            <h4>Total Checked Folders (By Manager) This</h4>
          ...
          {data.length > 0 ? (
            ...
            <p>No folders checked this {filter === 'month' ? 'month' : 'date range'}.</p>
          )}
          </div>
        </div>
        ...
        <div className="graph-chart"> 
          {data.length > 0 ? (
           <CFM_Graph key={data.length} data={data}/>
          ) : (
            <CFM_Graph key={0} data={[]}/>
          )}
        </div>
    </>
  )
